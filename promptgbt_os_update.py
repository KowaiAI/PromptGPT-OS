...
class PromptGBTOS:
    def __init__(self):
        ...
        self.question_templates = {
            ...
            # Complete all remaining question sets for every subcategory
            
            # For subcategories that don't have specific templates, generate comprehensive questions
            'Mobile App': [
                "What platform should the app target (iOS, Android, cross-platform)?",
                "What is the main functionality of your mobile app?",
                "Who is your target user demographic?",
  ...
  ██         ██     ██   ██████   ██  ██  ██ ██           ██     █████████ █████████   ██   
    ...
    def generate_questions_for_subcategory(self, subcategory: str) -> List[str]:
        ...
        # Generate comprehensive questions based on subcategory type
        base_questions = [
            f"What is the primary purpose of your {subcategory.lower()} project?",
            f"Who is your target audience for this {subcategory.lower()}?",
            f"What style or aesthetic do you want for your {subcategory.lower()}?",
            f"What is the intended length or duration of your {subcategory.lower()}?",
            f"What mood or emotion should your {subcategory.lower()} convey?",
            f"What key features or elements must be included in your {subcategory.lower()}?",
            f"What is your experience level with creating {subcategory.lower()} content?",
            f"What tools or software do you prefer for {subcategory.lower()} creation?",
            f"What is your budget or resource constraints for this {subcategory.lower()}?",
            f"What is the deadline or timeline for your {subcategory.lower()}?",
            f"Where will your {subcategory.lower()} be published or distributed?",
            f"What format should the final {subcategory.lower()} be delivered in?",
            f"What inspiration or references do you have for your {subcategory.lower()}?",
            f"What makes your {subcategory.lower()} different from competitors?",
            f"What specific technical requirements does your {subcategory.lower()} need?",
            f"What accessibility considerations are important for your {subcategory.lower()}?",
            f"What cultural or regional considerations should be included?",
            f"What language(s) should your {subcategory.lower()} support?",
            f"What age rating or content guidelines should your {subcategory.lower()} follow?",
            f"What call-to-action or next steps should your {subcategory.lower()} include?",
            f"What metrics will you use to measure the success of your {subcategory.lower()}?",
            f"What legal or compliance considerations affect your {subcategory.lower()}?",
            f"What branding elements should be incorporated in your {subcategory.lower()}?",
            f"What seasonal or time-sensitive elements are relevant to your {subcategory.lower()}?",
            f"What collaboration or team requirements do you have for your {subcategory.lower()}?",
            f"What research or preparation has been done for your {subcategory.lower()}?",
            f"What testing or feedback process will you use for your {subcategory.lower()}?",
            f"What updates or maintenance will your {subcategory.lower()} require?",
            f"What scalability considerations are important for your {subcategory.lower()}?",
            f"What user experience goals do you have for your {subcategory.lower()}?",
            f"What performance requirements must your {subcategory.lower()} meet?",
            f"What integration needs does your {subcategory.lower()} have with other systems?",
            f"What security considerations are important for your {subcategory.lower()}?",
            f"What content moderation or review process will your {subcategory.lower()} need?",
            f"What analytics or tracking should be implemented in your {subcategory.lower()}?",
            f"What monetization strategy applies to your {subcategory.lower()}?",
            f"What intellectual property considerations affect your {subcategory.lower()}?",
            f"What version control or change management does your {subcategory.lower()} need?",
            f"What documentation requirements does your {subcategory.lower()} have?",
            f"What training or support materials should accompany your {subcategory.lower()}?",
            f"What backup or recovery plan does your {subcategory.lower()} need?",
            f"What quality assurance standards should your {subcategory.lower()} meet?",
            f"What environmental or sustainability considerations apply to your {subcategory.lower()}?",
            f"What innovation or cutting-edge features should your {subcategory.lower()} include?",
            f"What community or social aspects should your {subcategory.lower()} incorporate?",
            f"What future-proofing considerations are important for your {subcategory.lower()}?",
            f"What cross-platform compatibility does your {subcategory.lower()} require?",
            f"What personalization features should your {subcategory.lower()} offer?",
            f"What data privacy considerations affect your {subcategory.lower()}?",
            f"What specific industry standards must your {subcategory.lower()} comply with?",
            f"What is the ultimate impact or outcome you want your {subcategory.lower()} to achieve?"
        ]
...
PROJECT OVERVIEW:
- Type: {project_type}
- Category: {self.current_category.title()} > {self.current_subcategory}
- Output: Professional {output_type}

DEVELOPMENT REQUIREMENTS:
"""
        for question, answer in list(answers.items())[:15]:
...
"""
        for question, answer in list(answers.items())[15:30]:
            prompt += f"• {question}: {answer}\n"
...
"""
        for question, answer in list(answers.items())[30:]:
            prompt += f"• {question}: {answer}\n"
...
2. Implementation guide and setup instructions
3. Testing procedures and validation
4. Documentation and user guides
5. Deployment and maintenance instructions

Generate a professional {output_type} that meets all specified requirements and follows industry best practices."""
...
"""
        for question, answer in list(answers.items())[20:40]:
            prompt += f"• {question}: {answer}\n"
...
"""
        for question, answer in list(answers.items())[40:]:
            prompt += f"• {question}: {answer}\n"
        
        prompt += f"""

CREATIVE DELIVERABLES:
1. High-resolution final artwork
2. Multiple format versions (PNG, JPG, etc.)
3. Color variations (if applicable)
4. Source files for future editing
5. Usage guidelines and specifications

Create a professional {image_type.lower()} that captures attention and achieves the intended visual impact."""
...
"""
        for question, answer in list(answers.items())[15:35]:
            prompt += f"• {question}: {answer}\n"
...
"""
        for question, answer in list(answers.items())[35:]:
            prompt += f"• {question}: {answer}\n"
        
        prompt += f"""

AUDIO DELIVERABLES:
1. Master track in multiple formats
2. Instrumental and vocal stems
3. MIDI files (if applicable)
4. Lyrics and chord charts
5. Production notes and metadata

Create a professional {music_type.lower()} that meets industry standards and achieves the desired artistic vision."""
...
1. Complete, polished content
2. Multiple format versions
3. SEO and optimization elements
4. Supporting materials (if needed)
5. Revision guidelines

Create professional {text_type.lower()} that engages the target audience and achieves the specified objectives."""
...
VIDEO DELIVERABLES:
1. Final edited video in specified format
2. Raw footage and source files
3. Script and storyboard materials
4. Multiple resolution versions
5. Thumbnail and promotional materials

Create a professional {video_type.lower()} that tells a compelling story and achieves the intended impact."""
    ...
    def run_questionnaire(self):
        ...
        while True:
            ...
            if self.handle_navigation(user_input):
                ...
                self.show_copyable_prompt(prompt)
            elif user_input.lower() == 'restart':
